...
func DmlHandler[Req libQuery.DmlModel](
	...
	return func(c context.Context) {
		defer func() {
			...
			if r := recover(); r != nil {
				core.Responder().HandleErrorState(libError.Join(r.(error), "error in Dml"), http.StatusInternalServerError, response.SYSTEM_FAULT, response.SYSTEM_FAULT_DESC, w)
				panic(r)
			}
		}()
		w := libContext.InitContext(c)
		...
		for _, command := range preControl[key] {
			...
			if errPreControl != nil {
				core.Responder().HandleErrorState(libError.Join(errPreControl, "PreControl"), http.StatusBadRequest, errPreControl.Description, errPreControl.Message, w)
				return
			}
		}
		...
		for _, command := range dml[key] {
			...
			if errInsert != nil {
				core.Responder().HandleErrorState(libError.Join(errInsert, "Insert"), http.StatusBadRequest, errInsert.Description, errInsert.Message, w)
				return
			}
